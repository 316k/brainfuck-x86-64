...
#       push    %rsp
		...
		mov		$0, %r10 # compteur de digits

		cmp		$0, %rax
		jge		print_word_dec_loop
		# Nombre négatif
		push	$'-'
		call	putchar

		# Négation de rax (complément à 2)
		neg		%rax
		
print_word_dec_loop:
		# Prépare le terrain pour le stuff
		mov 	$0, %rdx
		mov		$10, %rbx
		div		%rbx
		...
		cmp		$0,%rax
		jz		print_word_dec_print_loop
		jmp		print_word_dec_loop

print_word_dec_print_loop:

		call	putchar
		dec		%r10
		
		cmp		$0, %r10
		jz		print_word_dec_end
		jmp		print_word_dec_print_loop
